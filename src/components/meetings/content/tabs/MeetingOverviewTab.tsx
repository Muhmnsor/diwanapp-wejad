  ...
  return (
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {meeting.location && (
          <Card className="overflow-hidden border-0 shadow-md bg-gradient-to-r from-indigo-50 to-purple-50">
            <CardContent className="p-0">
              <div className="flex items-center p-4 gap-3">
                <div className="bg-primary/10 p-3 rounded-full">
        ...
        {meeting.folder_name && (
          <Card className="overflow-hidden border-0 shadow-md bg-gradient-to-r from-amber-50 to-yellow-50">
            <CardContent className="p-0">
              <div className="flex items-center p-4 gap-3">
                <div className="bg-primary/10 p-3 rounded-full">
                ...
                <div>
                  <p className="text-sm text-gray-500 mb-1">المجلد</p>
      ...
      <Card className="border-0 shadow-md overflow-hidden bg-gradient-to-r from-blue-50 to-indigo-50">
        ...
        <CardContent className="pt-4">
          {isLoadingObjectives ? (
            ...
            <p className="text-gray-500 py-4 text-center bg-white/50 rounded-md">لا توجد أهداف محددة لهذا الاجتماع</p>
          ) : (
            <ol className="list-decimal marker:text-primary list-inside space-y-3 pr-2">
              {objectives.map((objective) => (
                <li key={objective.id} className="text-gray-800 bg-white/50 p-3 rounded-md shadow-sm border-r-4 border-primary/40 hover:border-primary transition-colors">
                  {objective.content}
                </li>
              ))}
            </ol>
      ...
      <Card className="border-0 shadow-md overflow-hidden bg-gradient-to-r from-indigo-50 to-purple-50">
        ...
        <CardContent className="pt-4">
          {isLoadingAgenda ? (
            ...
            <p className="text-gray-500 py-4 text-center bg-white/50 rounded-md">لا توجد بنود في جدول الأعمال</p>
          ) : (
            <ol className="list-decimal marker:text-primary list-inside space-y-3 pr-2">
              {agendaItems.map((item) => (
                <li key={item.id} className="text-gray-800 bg-white/50 p-3 rounded-md shadow-sm border-r-4 border-purple-300 hover:border-purple-500 transition-colors">
                  {item.content}
                </li>
              ))}
            </ol>
