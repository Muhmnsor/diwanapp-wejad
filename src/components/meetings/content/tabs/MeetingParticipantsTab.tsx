...
const participantFormSchema = z.object({
  userEmail: z.string().email({ message: "البريد الإلكتروني غير صالح" }),
  userDisplayName: z.string().min(2, { message: "يجب أن يكون الاسم أكثر من حرفين" }),
...
export const MeetingParticipantsTab: React.FC<MeetingParticipantsTabProps> = ({
  ...
  const { users, isLoading: isUsersLoading } = useUsersList();
  const { mutate: addParticipant, isPending: isAddingParticipant } = useAddMeetingParticipant();
  ...
  const handleUserSelect = (user: {id: string; display_name: string; email: string}) => {
                ...
                <TableBody>
                  {participants.map((participant) => (
                    <TableRow key={participant.id}>
                      ...
                      <TableCell>
                        <Select
                          defaultValue={participant.attendance_status}
                          onValueChange={(value) => handleUpdateAttendance(participant.id, value)}
                        >
                          <SelectTrigger className="w-[120px]">
      ...
      <Sheet open={isParticipantSheetOpen} onOpenChange={setIsParticipantSheetOpen}>
        <SheetContent className="sm:max-w-md" dir="rtl">
          <SheetHeader>
            ...
            <SheetDescription>
              يمكنك اختيار مستخدم موجود أو إضافة مشارك يدويًا
            </SheetDescription>
          </SheetHeader>
          ...
          <div className="py-4">
            <Form {...form}>
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                {!selectedUser && (
                  <div className="mb-6">
                    <h3 className="text-sm font-medium mb-2">المستخدمون في النظام</h3>
                    <div className="max-h-[200px] overflow-y-auto border rounded-md p-2">
                      {isUsersLoading ? (
                        <div className="py-4 text-center">جاري تحميل المستخدمين...</div>
                      ) : users.length === 0 ? (
                ...
                <SheetFooter className="flex justify-between gap-2 mt-6" dir="ltr">
                  <Button 
                    type="button" 
                    variant="outline" 
                  ...
                  <Button 
                    type="submit" 
                    disabled={isAddingParticipant || !form.formState.isValid}
                  >
                    {isAddingParticipant ? "جاري الإضافة..." : "إضافة مشارك"}
                  </Button>
                </SheetFooter>
              </form>
            </Form>
