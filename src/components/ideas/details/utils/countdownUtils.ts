...
// دالة مساعدة جديدة لاستخراج إجمالي الساعات من فترة المناقشة
export const extractTotalHours = (discussion_period: string | undefined): number => {
  if (!discussion_period) return 0;
...
// دالة لتنسيق إجمالي الساعات بشكل قابل للقراءة
export const formatTotalPeriod = (totalHours: number): string => {
  if (totalHours <= 0) return "غير محددة";
