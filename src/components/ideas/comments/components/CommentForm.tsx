  ...
  if (!isDiscussionActive) {
    return (
      <Alert variant="destructive" className="bg-amber-50 border-amber-300 text-amber-800">
        ...
        <AlertTitle className="text-sm sm:text-base text-amber-800 mr-1 sm:mr-2">انتهت فترة المناقشة</AlertTitle>
        <AlertDescription className="text-xs sm:text-sm text-amber-700 mr-1 sm:mr-2">
      ...
      <div className="flex-1 w-full">
        <div className="relative">
          <Textarea 
            ...
            onFocus={onFocus} 
            className="min-h-[60px] sm:min-h-[80px] w-full resize-none border rounded-xl sm:rounded-2xl focus-visible:ring-0 text-right px-[18px] sm:px-[23px] py-3 sm:py-4 pr-[40px] sm:pr-[50px] text-sm sm:text-base" 
          />
        ...
        <div className="flex justify-between items-center mt-1 sm:mt-2">
          <div className="flex gap-1 sm:gap-2 items-center">
            {selectedFile && <div className="flex items-center gap-1 text-xs sm:text-sm text-muted-foreground">
                <span className="truncate max-w-[100px] sm:max-w-none">{selectedFile.name}</span>
