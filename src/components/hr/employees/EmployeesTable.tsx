...
export function EmployeesTable({ employees, isLoading }: EmployeesTableProps) {
  const [viewDialogEmployee, setViewDialogEmployee] = useState<any | null>(null);
  const [editDialogEmployee, setEditDialogEmployee] = useState<any | null>(null);
  const [deleteDialogEmployee, setDeleteDialogEmployee] = useState<any | null>(null);
  ...
  return (
    <>
              ...
              <TableCell>
                <DropdownMenu>
                  ...
                  <DropdownMenuContent align="end">
                    <DropdownMenuItem onClick={() => setViewDialogEmployee(employee)}>
                      <Eye className="ml-2 h-4 w-4" />
                      <span>عرض</span>
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setEditDialogEmployee(employee)}>
      ...
      {viewDialogEmployee && (
        <ViewEmployeeDialog 
          employee={viewDialogEmployee} 
          isOpen={!!viewDialogEmployee}
          onClose={() => setViewDialogEmployee(null)} 
        />
      )}
      ...
      {editDialogEmployee && (
        <EditEmployeeDialog 
          ...
          isOpen={!!editDialogEmployee}
          onClose={() => setEditDialogEmployee(null)}
          onSuccess={() => setEditDialogEmployee(null)} 
        />
      )}
      ...
      {deleteDialogEmployee && (
        <DeleteEmployeeDialog 
          ...
          employeeName={deleteDialogEmployee.full_name}
          isOpen={!!deleteDialogEmployee}
          onClose={() => setDeleteDialogEmployee(null)}
          onSuccess={() => setDeleteDialogEmployee(null)} 
        />
      )}
    </>
  );
