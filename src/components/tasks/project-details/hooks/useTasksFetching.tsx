...
      // Add the requested ordering: first by order_position, then by created_at
      query = query
        .order('order_position', { ascending: true, nullsLast: true })
        .order('created_at', { ascending: true });

      const { data, error } = await query; // Await after chaining all methods
...
      } else {
            // For workspace, meeting, or general tasks, stages might not be relevant for grouping
            // or they might be displayed in a flat list, so we clear the tasksByStage
            setTasksByStage({});
        }
