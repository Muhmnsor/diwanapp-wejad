...
export const VoucherViewDialog = ({
  open,
  onOpenChange,
  voucher,
        ...
        <DialogHeader>
          <div className="flex justify-between items-center">
            ...
            <DialogTitle className="text-right">
              سند صرف رقم: {voucher.voucher_number}
            </DialogTitle>
          </div>
        </DialogHeader>
        ...
        <div className="print:p-10">
          <div className="flex flex-col sm:flex-row justify-between mb-8">
            ...
            <div className="text-right">
              <div className="font-medium">التاريخ: {format(new Date(voucher.date), "yyyy/MM/dd")}</div>
              <div className="font-medium">طريقة الدفع: {getPaymentMethodLabel(voucher.payment_method)}</div>
              {voucher.payment_method === "check" && voucher.check_number && (
                <div className="font-medium">رقم الشيك: {voucher.check_number}</div>
              )}
              {voucher.payment_method === "bank_transfer" && voucher.bank_account_number && (
                <div className="font-medium">رقم الحساب: {voucher.bank_account_number}</div>
              )}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-8">
            ...
            <div className="text-right">
              <h3 className="font-bold mb-2">بيانات المستفيد</h3>
              <div>الاسم: {voucher.beneficiary_name}</div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            <div className="text-right">
              <div className="border-t pt-4">
                <h4 className="font-bold mb-2">المستفيد:</h4>
            ...
            <div className="text-right">
              <div className="border-t pt-4">
                <h4 className="font-bold mb-2">المحاسب:</h4>
                <div className="h-16"></div>
                <p>الاسم والتوقيع</p>
              </div>
            </div>
