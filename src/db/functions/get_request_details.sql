...
BEGIN
  ...
  SELECT 
    jsonb_build_object(
      ...
      'permissions', jsonb_build_object(
        'is_requester', (rd.requester_id = v_current_user_id),
        'is_admin', v_is_admin,
        'can_view', (rd.requester_id = v_current_user_id OR v_is_admin OR EXISTS (
          SELECT 1 FROM workflow_steps ws
          WHERE ws.workflow_id = rd.workflow_id
  ...
  IF v_result IS NULL THEN
    RAISE EXCEPTION 'Request not found or you do not have permission to view it';
  END IF;
...
-- Add comment
COMMENT ON FUNCTION public.get_request_details(uuid) IS 'Get detailed information about a request including the workflow, current step, and permissions';
